...
plot_T = dittoHeatmap(spe_subsetT[, cur_cells], 
             genes = c("CD3", "CD8", "CD4", "FOXP3", "Ki67"),
...
plot_MNP = dittoHeatmap(spe_subsetMNP[, cur_cells], 
             genes = c("HLADR", "CD1c", "Langerin", "CD11c", "CD303", "CD14", "CD163", "FXIIIa", "Siglec1", "CD11b"),
             assay = "norm_exprs", scale = "none",
             heatmap.colors = viridis(100), 
             cluster_rows = FALSE,
             annot.by = c("celltype"),
...
```{r Figure 1C, fig.height=5, fig.width=8}
df = data.frame(
...
```{r Figure 2A, fig.height=4, fig.width=8}
spe_subset %>%
  colData() %>%
  as.data.frame() %>%
  group_by(imShort) %>%
...
```{r Figure 2B, fig.height=7, fig.width=5}
...
plot_df %>%
  ...
  mutate(broad_type = factor(broad_type, levels = c("B cell", "T cell", "MNP"))) %>%
  group_by(broad_type) %>%
  summarise(mean = mean(LAInnerDist_std), sem = sd(LAInnerDist_std)/sqrt(n())) %>%
  ggplot(aes(x = broad_type, y = mean, fill = broad_type)) +  
    geom_bar(stat = "identity", colour = "black") +
    geom_errorbar(aes(x=broad_type, ymin=(mean - 2.5*sem), ymax=(mean + 2.5*sem)), width=0.4) +
    theme_bw() +
    labs(x = "Cell Type", y = "Mean Standardised Distance from LA Edge") +
    theme(plot.title = element_text(hjust = 0.5), legend.position = "none") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.3))) +
    scale_fill_manual(values = c("orange", "dodgerblue", "darkolivegreen4")) +
    theme(text = element_text(size = 18)) +
...
```{r Figure 2C, fig.height=8, fig.width=6}
spe_subset %>%
  colData() %>%
  as.data.frame() %>%
  drop_na() %>%
...
kruskal.test(LAInnerDist_std ~ celltype, data = plot_df)
pairwise.wilcox.test(
  plot_df$LAInnerDist_std,
  plot_df$celltype,
...
```{r Figure 3A, fig.width=9, fig.height=7}
plot_df = spe_subset %>%
  colData() %>%
  as.data.frame() %>%
  left_join(dist_df, by = "imShort") %>%
...
summary_df = combined_results %>%
  ...
  select(imShort, mean_T, mean_B, mean_MNP) %>%
  pivot_longer(cols = c(mean_T, mean_B, mean_MNP)) 

anova = aov(value ~ name, data = summary_df)
summary(anova)
em = emmeans(anova, ~ name)
contrast(em, method = "pairwise", adjust = "tukey") 
```

```{r Figure 4A, fig.height=7, fig.width=8}
...
```{r Figure 4B, fig.height=5.5, fig.width=6.5}
plot_df = spe_subset %>% 
  colData() %>% 
  as.data.frame() %>%
  dplyr::filter(celltype %in% c("Lang- cDC2", "Lang+ cDC2", "DC3", "Macrophage"))  %>%
...
plot_df %>%
  ...
  mutate(celltype = factor(celltype, levels = c("Lang- cDC2", "Lang+ cDC2", "DC3", "Macrophage"))) %>%
  ggplot(aes(x=celltype, y=mean, fill=celltype)) +
    geom_bar(stat = "identity", colour = "black") +
    geom_errorbar(aes(x=celltype, ymin=(mean - sem), ymax=(mean + sem), width=0.4)) +
    theme_bw() +
